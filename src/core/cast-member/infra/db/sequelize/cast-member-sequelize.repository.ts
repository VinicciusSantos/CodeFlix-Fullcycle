...
export class CastMemberSequelizeRepository implements ICastMemberRepository {
  ...
  async insert(entity: CastMember): Promise<void> {
    await this.castMemberModel.create(entity.toJSON() as CreateOptions);
  }

  async bulkInsert(entities: CastMember[]): Promise<void> {
    await this.castMemberModel.bulkCreate(entities.map((e) => e.toJSON()) as BulkCreateOptions[]);
  }
  ...
  async update(entity: CastMember): Promise<void> {
    ...
    const [affectedRows] = await this.castMemberModel.update(entity.toJSON() as Attributes<CastMemberModel>, {
