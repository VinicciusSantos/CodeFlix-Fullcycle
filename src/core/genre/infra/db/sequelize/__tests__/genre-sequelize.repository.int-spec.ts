...
import {
  ...
  CategoryModel,
} from '@core/category/infra/db/sequelize';
  ...
  describe('search method tests', () => {
    it('should order by created_at DESC when search params are null', async () => {
      ...
      const genres = Genre.fake()
        ...
        .build();
      await genreRepo.bulkInsert(genres);
      const spyToEntity = jest.spyOn(GenreModelMapper, 'toAggregate');
      ...
      [...genres.slice(1, 16)].reverse().forEach((
        item,
        index,
      ) => {
