...
export class VideoSequelizeRepository implements IVideoRepository {
  ...
  public async insert(entity: Video): Promise<void> {
  ...
  public async bulkInsert(entities: Video[]): Promise<void> {
  ...
  public async findByIds(ids: VideoId[]): Promise<Video[]> {
    const models = await this.videoModel.findAll({
      where: {
        video_id: {
  ...
  public async update(entity: Video): Promise<void> {
  ...
  public async search(props: VideoSearchParams): Promise<VideoSearchResult> {
