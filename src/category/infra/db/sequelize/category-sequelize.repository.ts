...
export class CategorySequelizeRepository
  ...
  public async update(entity: Category): Promise<void> {
    ...
    await this.categoryModel.update(
      ...
      { returning: false, where: { category_id: id } },
    );
  }
  ...
  public async delete(category_id: Uuid): Promise<void> {
    ...
    await this.categoryModel.destroy({ where: { category_id: category_id.id } });
  }

  public async search(props: CategorySearchParams): Promise<CategorySearchResult> {
    ...
    const { rows, count } = await this.categoryModel.findAndCountAll({
      ...
      order: [sort && this.sortableFields.includes(sort) ? [sort, sort_dir!] : ["created_at", "desc"]],
      limit: per_page,
      offset,
    });
